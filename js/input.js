function addKeyListeners() {
	...
	jQuery("#pauseBtn").on('touchstart mousedown', function() {
	...
	jQuery("#colorBlindBtn").on('touchstart mousedown', function() {
	...
	if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
			jQuery("#restart").on('touchstart', function() {
			init();
			canRestart = false;
			jQuery("#gameoverscreen").fadeOut();
		});
		...
		jQuery("#restart").on('mousedown', function() {
			init();
			canRestart = false;
			jQuery("#gameoverscreen").fadeOut();
		});
	...
	if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
			jQuery("#restartBtn").on('touchstart', function() {
			init(1);
			canRestart = false;
			jQuery("#gameoverscreen").fadeOut();
		});
		...
		jQuery("#restartBtn").on('mousedown', function() {
			init(1);
			canRestart = false;
			jQuery("#gameoverscreen").fadeOut();
		});
...
function handleClickTap(x,y) {
	if (x < 120 && y < 83 && jQuery('.helpText').is(':visible')) {
		showHelp();
		return;
	}
