...
function showText(text) {
	...
	if (text == 'gameover') {
	   //Clay('client.share.any', {text: 'Think you can beat my score of '+ score + ' in Super Cool Game?'})
		jQuery("#gameoverscreen").fadeIn();
    	}
	jQuery(".overlay").html(messages[text]);
	jQuery(".overlay").fadeIn("1000", "swing");
...
function hideText() {
	jQuery(".overlay").fadeOut(150, function() {
		jQuery(".overlay").html("");
	})
}
...
function gameOverDisplay() {
	...
	jQuery("#gameoverscreen").fadeIn();
	jQuery("#buttonCont").fadeIn();
	jQuery("#gamecontainer").fadeIn();
...
function updateHighScores (){
    jQuery("#cScore").text(score);
    jQuery("#1place").text(highscores[0]);
    jQuery("#2place").text(highscores[1]);
    jQuery("#3place").text(highscores[2]);
}
...
function pause(o) {
	...
	var c = document.getElementById("canvas");
	if (gameState == -1) {
		jQuery('#fork-ribbon').fadeOut(300, 'linear');
		jQuery('#restartBtn').fadeOut(300, "linear");
		jQuery('#buttonCont').fadeOut(300, "linear");
		...
		jQuery('#restartBtn').fadeIn(300, "linear");
		jQuery('#buttonCont').fadeIn(300, "linear");
		jQuery('.helpText').fadeIn(300, 'linear');
		if (message == 'paused') {
			showText(message);
		}
