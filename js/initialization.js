function initialize(a) {
	...
	jQuery('#clickToExit').bind('click', toggleDevTools);
	window.settings;
	if (/Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        jQuery('.rrssb-email').remove();
		settings = {
			os: "other",
			platform: "mobile",
			startDist: 227,
	...
	if (a != 1) {
		...
		if (settings.platform == 'mobile') {
			jQuery('#startBtn').on('touchstart', startBtnHandler);
		} else {
			jQuery('#startBtn').on('mousedown', startBtnHandler);
		}

		document.addEventListener('touchmove', function(e) {
			e.preventDefault();
		}, false);
		jQuery(window).resize(scaleCanvas);
...
function startBtnHandler() {
	...
	if (jQuery('#openSideBar').is(':visible')) {
