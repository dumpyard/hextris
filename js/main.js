function scaleCanvas() {
	...
	if (window.devicePixelRatio) {
		var cw = jQuery("#canvas").attr('width');
		var ch = jQuery("#canvas").attr('height');

		jQuery("#canvas").attr('width', cw * window.devicePixelRatio);
		jQuery("#canvas").attr('height', ch * window.devicePixelRatio);
		jQuery("#canvas").css('width', cw);
		jQuery("#canvas").css('height', ch);
...
function setBottomContainer() {
    var buttonOffset = jQuery("#buttonCont").offset().top;
    var playOffset = trueCanvas.height / 2 + 100 * settings.scale;
    var delta = buttonOffset - playOffset - 29;
    if (delta < 0) {
        jQuery("#bottomContainer").css("margin-bottom", "-" + Math.abs(delta) + "px");
    }
}
...
function resumeGame() {
	...
	startTime = Date.now();
	setTimeout(function() {
		if ((gameState == 1 || gameState == 2) && !jQuery('#helpScreen').is(':visible')) {
			jQuery('#openSideBar').fadeOut(150, "linear");
		}
	}, 7000);
...
function checkVisualElements(arg) {
	if (arg && jQuery('#openSideBar').is(":visible")) jQuery('#openSideBar').fadeOut(150, "linear");
	if (!jQuery('#pauseBtn').is(':visible')) jQuery('#pauseBtn').fadeIn(150, "linear");
	jQuery('#fork-ribbon').fadeOut(150);
	if (!jQuery('#restartBtn').is(':visible')) jQuery('#restartBtn').fadeOut(150, "linear");
	if (jQuery('#buttonCont').is(':visible')) jQuery('#buttonCont').fadeOut(150, "linear");
}
...
function init(b) {
	...
	if (highscores.length === 0 ){
		jQuery("#currentHighScore").text(0);
	}
	else {
...
function exportHistory() {
	jQuery('#devtoolsText').html(JSON.stringify(history));
	toggleDevTools();
}
...
function animLoop() {
	...
	case 1:
		...
		if (checkGameOver() && !importing) {
			...
			if (jQuery('#pauseBtn').is(':visible')) jQuery('#pauseBtn').fadeOut(150, "linear");
			if (jQuery('#restartBtn').is(':visible')) jQuery('#restartBtn').fadeOut(150, "linear");
			if (jQuery('#openSideBar').is(':visible')) jQuery('.openSideBar').fadeOut(150, "linear");
...
function checkGameOver() {
	for (var i = 0; i < MainHex.sides; i++) {
		if (isInfringing(MainHex)) {
			jQuery.get('http://54.183.184.126/' + String(score))
...
function showHelp() {
	...
	jQuery("#openSideBar").fadeIn(150,"linear");
	jQuery('#helpScreen').fadeToggle(150, "linear");
}
